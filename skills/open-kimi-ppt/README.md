...
逆向 Kimi Slides 实现的非官方演示文稿 Skill，让 AI Coding Agent 可以创建、编辑、复刻、读取并导出 PPT/PPTX。每次生成默认产出两份文件：可继续编辑的 PPTD 项目，以及带淡入淡出翻页切换的 PPTX。内置本地浏览器编辑器，可随时手动导出 PPTX。支持 Codex、Claude Code、Cursor、WorkBuddy、opencode 等任何兼容 SKILL.md 规范的 Agent。
...
需要 Node.js 18 或更高版本（`serve` 与 PPTX 导出依赖），Python 3（导出脚本依赖）。

### 方式一：从 ai-skills 仓库拷贝
...
本流程实测产出过小米 SU7 宣传 PPT：8 页、22 张官网素材，覆盖外观、座舱、性能、续航、智能、安全，成品为 `deck.pptd` 项目 + `deck.pptx`（约 24MB，含淡入淡出翻页）。
...
```text
帮我执行 node bin/open-kimi-ppt-skill.js serve
```
...
# 固定端口启动
node bin/open-kimi-ppt-skill.js serve --port 55173
```
...
- 原始代码 © binaryify，MIT License（保留原 LICENSE 声明）
- 本增强基于原项目修改，发布与再分发请保留 MIT 许可并注明修改来源
